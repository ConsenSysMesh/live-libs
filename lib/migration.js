...
function deploy(register, web3, onTestrpc) {
  ...
  return deployLiveLibsContract(register, web3, onTestrpc).then(function() {
    if (!fs.existsSync(fileUtils.dataFilePath))
...
function deployLiveLibsContract(register, web3, onTestrpc) {
  var source = fs.readFileSync('./contracts/LiveLibs.sol', 'utf8');
